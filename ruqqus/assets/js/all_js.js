	...
	function loadEmojis() {
		...
		for (i=0; i < emojis.length; i++) {
			...
			for (j=0; j < arr.length; j++) { 
				str += `<button class="btn m-1 p-3" onclick="getEmoji(\'${arr[j]}\')" style="width:40px; overflow: hidden;" data-toggle="tooltip" title=":${arr[j]}:" delay:="0"><img width=30 src="/assets/images/emojis/${arr[j]}.gif" alt="${arr[j]}-emoji"/></button>`;
			}
